...
class NumericVersionSegment(int):
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"{repr(self.__NumericVersionSegment_value__)}>"
        )
...
class AlphanumericVersionSegment(str):
    ...
    def __new__(cls, value: Optional[str], format: str):
        ...
        if value is None:
            value = ""
        if any(not v.isalnum() and v != "-" for v in value):
            raise ValueError(
                f"{cls.__name__} must may only cointain alphanumeric characters or '-', got {value}"
            )
        return super().__new__(cls, value)
